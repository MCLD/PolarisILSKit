...
if [[ $BLD_DEPLOY == true && -f docker-deploy.bash ]]; then
  echo "NUGET_PROJECT_NAME=$NUGET_PROJECT_NAME" > deploy.env
  echo "NUGET_APIKEY=$NUGET_APIKEY" >> deploy.env
  docker run --rm --name do-deploy --env-file deploy.env $BLD_DOCKER_IMAGE:$BLD_DOCKER_TAG
  rm deploy.env
fi
